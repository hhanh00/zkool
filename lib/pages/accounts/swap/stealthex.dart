...
class StealthExState extends State<StealthExPage> with WithLoadingAnimation {
  ...
  swap() async {
    final q = quote;
    if (q == null) return;
    final taddr = warp.getAccountAddress(aa.coin, aa.id, 0, 1);
    if (taddr.isEmpty) {
      await showMessageBox(context, s.error, s.noTaddr);
      return;
    }
  ...
  _send(String address, String amount) async {
    load(() async {
      ...
      try {
        ...
        final txPlan = await warp.pay(aa.coin, aa.id, payment);
...
Future<SwapT> createExchange(String rateId, String from, String to,
  ...
  warp.storeSwap(aa.coin, aa.id, swap);
